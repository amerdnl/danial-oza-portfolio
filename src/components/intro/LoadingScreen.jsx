...
export function LoadingScreen({ progress }) {
  ...
  return (
    ...
    >
      <p className="intro-fade-in text-2xl font-extrabold tracking-tight text-heading sm:text-3xl">
        {advisor.name}
      </p>
      <p
        className="intro-fade-in mt-2 text-xs font-medium uppercase tracking-[0.28em] text-muted"
      ...
      <div
        aria-hidden="true"
        className="intro-fade-in mt-9 h-px w-full overflow-hidden rounded-full bg-border"
        style={{ animationDelay: '0.24s' }}
      >
        <div
          className="h-full rounded-full bg-brand shadow-[0_0_8px_0_rgba(149,1,1,0.35)] transition-[width] duration-200 ease-out"
      ...
      <p
        aria-hidden="true"
        className="intro-fade-in mt-4 font-mono text-xs tabular-nums text-muted"
        style={{ animationDelay: '0.24s' }}
      >
